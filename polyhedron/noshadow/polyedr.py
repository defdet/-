...
class Polyedr:
    ...
    # Метод для подсчета суммы длин проекций рёбер,
    # оба из концов которых — "хорошие" точки
    def calculate_good_edge_lengths(self):
        good_edge_lengths = 0.0
        good_edge_xy = 0.0
        for e in self.edges:
            first_check = self.is_good_point(e.beg)
            second_check = self.is_good_point(e.fin)
            if first_check and second_check:  # Оба конца являются "хорошими"
                edge = (e.fin - e.beg)  # "Вектор ребра"
                good_edge_lengths += sqrt(edge.dot(edge))
                good_edge_xy += sqrt(edge.x ** 2 + edge.y ** 2)
    ...
    def draw(self, tk):
        ...
        print(f"Сумма длин 'хороших' ребер равна {lengts_sum}.\
               Сумма длин проекций ребер равна {xy_sum}")
