...
class Polyedr:
    ...
    def is_good_projection(self, point):
        return abs(point.x) <= 0.5 and abs(point.y) <= 0.5

    def good_projection_length(self, edge):
        second_check = self.is_good_projection(edge.fin)
        first_check = self.is_good_projection(edge.beg)
        return edge.fin - edge.beg if first_check and second_check else 0.0

    def sum_good_projection_lengths(self):
        return sum(self.good_projection_length(e) for e in self.edges)

    # Метод изображения полиэдра
    def draw(self, tk):
        tk.clean()
        good_projection_sum = self.sum_good_projection_lengths()
        print(f"Sum of lengths of good projections: {good_projection_sum:.4f}")
        for e in self.edges:
            for s in e.gaps:
